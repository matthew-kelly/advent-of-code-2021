const run1 = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    input[i].split('').map((num, index) => {
      onesCount[index] =
        num === '1' ? (onesCount[index] += 1) : onesCount[index];
    });
  }
  ...
  return (
    parseInt(gammaDigits.join(''), 2) * parseInt(epsilonDigits.join(''), 2)
  );
};
...
const run2 = (input) => {
  ...
  return (
    parseInt(reduceList(oxygenList, 1, 'o'), 2) *
    parseInt(reduceList(co2List, 1, 'c'), 2)
  );
};
